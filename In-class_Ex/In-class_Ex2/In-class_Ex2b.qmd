...
-   p_folded_sim: the simulation folded \[0, 0.5\] range ranked p-value (based on [this](https://github.com/pysal/esda/blob/4a63e0b5df1e754b17b5f1205b cadcbecc5e061/esda/crand.py#L211-L213))
...
We can use **tmap** package to prepare a choropleth map using the values in the *p_ii_sim* field.
...
piisim_val <- tm_shape(lisa) +
  tm_fill("p_ii_sim") +
  tm_borders(alpha = 0.5) +
  tm_layout(main.title = "p-value of Local Moran's I", 
            main.title.size = 0.8)

tmap_arrange(ii_val, piisim_val, asp = 1, ncol = 2)
