...
left join with spatial layer (hunan) and nonspatial layer so that we can retain spatial geometric properties Do a selection to pick the relevant columns that we want, note that the geometry column was retained Did not specify the "by=" because there is a common column in both data frame (i.e., Country )
