...
Let's take a look at the study data that we have extracted:
...
Since there are no rows returned, it means that `origin_data` has no duplicates and we can proceed to the next step.

We will then update the `origin_data` data frame with the planning subzone codes found in `mpsz` using the following code chunk.
