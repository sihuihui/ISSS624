...
We see that there are some busstops outside the Singapore Map and we will remove them in the later steps for the purpose of this exercise. 


### Counts of Businesses, Financial Services and MRT Stations in Each Hexagon
...
glimpse(busmap)
```
We create `busmap_grid` to just have the grid_id, bs_count and geometry to use it for subsequent data wrangling steps. 
...
Unlike the businesses, Financial Services and MRT Exits, we cannot use the counts. So we will first derive the grid_ids for the residences in hdb_res_sf using st_interaction(). 
...
We will drop the geometry using st_drop_geometry so that hdb_res_hex becomes a tibble dataframe. After it is a tibble dataframe, we will group by the grid_id and sum the dwellings to find out 
...
Notice that `busgrid` has hexagon related information, but we are unable to map it back to the odbus_WDMP data because there are no common fields.

Hence, we will use st_intersection() to get the busstop codes and their corresponding grid_id.
...
We then combine busstop_hex and mpsz_hex to get the busstop number, their grid_id, the subzones they are in and the count of the various factors affecting commuter flows
...
We use the following code chunk to visualise the distribution of businesses, financial Services, MRT exits and HDB Residential Dwelling over the hexagon grid with busstops.
...
To visualise the commuter flows from origin busstops to destination busstops, we need to create an Origin-Destination (O-D) Matrix. First, we will convert the `busstop_hex_mpsz` (an sf dataframe) to a tibble dataframe using `st_drop_geometry()`.
...
Let us plot out the flowlines from the 95th percentile onwards to find out where the most commuter flows are going to.
...
flowline_95 <- flowline %>%
  filter(flowline$MORNING_PEAK >= 1411)
...
ggplot(flowline_95, aes(x=MORNING_PEAK))+
  geom_histogram(binwidth = 2000, color="#e9ecef", alpha=0.9)+
  xlim(1411, 77433)

```

From the above output, we see that the distribution of morning peak commuter flow is highly skewed with a wide range. The number of trips at the 95th percentile and above range from 8000+ trips to more than 80,000 trips.
...
tm_shape(busmap_grid) +
  ...
  tm_lines(lwd = "MORNING_PEAK",
           ...
           n = 6,
           alpha = 0.8)+
    tm_layout(main.title = "Flow for Passenger Volume more than 10,000 trips",
            main.title.size = 0.9,
            main.title.position = "center")
...
First we will use `as.Spatial()` to convert busmap_grid from sf tibble dataframe to Spatial Polygons Dataframe using sp package.
...
The print report above reveals that variables ORIGIN_BIZ_COUNT, ORIGIN_FINSERV_COUNT,ORIGIN_MRTEXIT_COUNT,
ORIGIN_SCHOOLS_COUNT, ORIGIN_DWELLINGS, DESTIN_BIZ_COUNT,
DESTIN_FINSERV_COUNT, DESTIN_MRTEXIT_COUNT, DESTIN_SCHOOLS_COUNT,
DESTIN_DWELLINGS  consist of 0 values.
...
#### Remove both variables and outliers 
